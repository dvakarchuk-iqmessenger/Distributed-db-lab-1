...
## Етап 3.6 – головний двигун програми (Engine)

На цьому етапі реалізовано єдиний «двигун» додатку, який керує всіма попередніми етапами на основі конфігурації.

- Створено YAML-файл `config/engine.yaml`, у якому задаються прапорці для виконання окремих кроків: `run_website_parser`, `run_save_to_csv`, `run_save_to_json`, `run_save_to_yaml`, `run_save_to_sqlite`, `run_save_to_mongodb`. 
- Реалізовано клас `Lab1::Engine`, який:
    - має атрибути `config`, `items`, `cart`, `db_connector`;
    - завантажує конфігурацію з `engine.yaml` методом `load_config`;
    - в методі `run` ініціалізує логування, завантажує конфіг, підключається до БД через `DatabaseConnector`, викликає необхідні методи згідно з конфігом і по завершенню закриває з’єднання з БД. 
- Метод `run_methods` проходиться по параметрах конфігурації та викликає відповідні методи класу:
    - `run_website_parser` – запускає `SimpleWebsiteParser`, наповнює колекцію `cart` об’єктами `Item`;
    - `run_save_to_csv` / `run_save_to_json` / `run_save_to_yaml` – зберігають дані з `cart` у файли CSV/JSON/YAML;
    - `run_save_to_sqlite` – створює (за потреби) таблицю в SQLite і записує туди всі елементи з кошика;
    - `run_save_to_mongodb` – зберігає колекцію товарів у колекцію `books` бази MongoDB (якщо обрано відповідний тип БД у `databaseconfig.yaml`).
- Файл `lib/main.rb` спрощено до єдиної точки входу: створення екземпляра `Lab1::Engine` та виклику `engine.run`, що забезпечує централізоване керування всією логікою застосунку. 



## Етап 4 – запуск додатку

На цьому етапі реалізовано єдину точку входу в застосунок та зручний спосіб запуску через Rake.

- Створено файл `main.rb` у корені проєкту, який:
    - підключає основні компоненти (`AppConfigLoader`, `Configurator`, `Engine`);
    - завантажує конфігурацію для Engine з `config/engine.yaml`;
    - створює екземпляр `Lab1::Configurator`, застосовує до нього параметри з YAML;
    - створює екземпляр `Lab1::Engine` та викликає `engine.run(configurator.config)`;
    - містить обробку виключень і виводить повідомлення у випадку помилки при старті. [file:74]
- У файлі `Rakefile` додано задачу для запуску проєкту:
    - таска `rake run` викликає `ruby lib/main.rb`, що дозволяє запускати весь додаток однією командою з командного рядка. [file:74]
- Після налаштування запуск можливий двома способами:
    - безпосередньо: `ruby lib/main.rb`;
    - через Rake: `rake run` (або `bundle exec rake run` у середовищі з Bundler). [file:74]
